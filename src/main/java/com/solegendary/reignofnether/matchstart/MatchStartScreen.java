...
public class MatchStartScreen extends Screen {
    ...
    private static final Faction[] FACTION_TILE_ORDER = { Faction.VILLAGERS, Faction.MONSTERS, Faction.PIGLINS, Faction.FORMIX, Faction.RANDOM };
    ...
    private void renderSlotRow(GuiGraphics g, StartPos sp, String localName,
        ...
        if (sp.enabled && (empty || mine)) {
            int rowHitRight = x + width - FRAME_SIZE * FACTION_TILE_ORDER.length - 2 * 2 - FRAME_SIZE - 6 - 8 - 16;
        ...
        int factionTileCount = FACTION_TILE_ORDER.length;
        int factionTotalW = FRAME_SIZE * factionTileCount + 2 * 2;
        ...
        for (Faction f : FACTION_TILE_ORDER) {
