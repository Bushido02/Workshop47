...
import com.solegendary.reignofnether.playerprogression.FormixPlayerProgressionClientEvents;
...
public class HudClientEvents {
            ...
            if (GameruleClient.scenarioMode) {
                ...
                if (!diffsButton.isHidden.get()) {
                    diffsButton.render(evt.getGuiGraphics(),
                            screenWidth - (StartButtons.ICON_SIZE * 14),
                            StartButtons.ICON_SIZE / 2,
                            mouseX,
                            mouseY
                    );
                    ...
                    if (!StartPosClientEvents.isEnabled()) {
                        if (!StartButtons.villagerStartButton.isHidden.get()) {
                            StartButtons.villagerStartButton.render(evt.getGuiGraphics(),
                                    screenWidth - (StartButtons.ICON_SIZE * 12),
                                    StartButtons.ICON_SIZE / 2,
                                    mouseX,
                                    mouseY
                            );
                        ...
                        if (!StartButtons.monsterStartButton.isHidden.get()) {
                            StartButtons.monsterStartButton.render(evt.getGuiGraphics(),
                                    (int) (screenWidth - (StartButtons.ICON_SIZE * 10)),
                                    StartButtons.ICON_SIZE / 2,
                                    mouseX,
                                    mouseY
                            );
                        ...
                        if (!StartButtons.formixStartButton.isHidden.get()) {
                            StartButtons.formixStartButton.render(evt.getGuiGraphics(),
                                    screenWidth - (StartButtons.ICON_SIZE * 6),
                                    StartButtons.ICON_SIZE / 2,
                                    mouseX,
                                    mouseY
                            );
                            renderedButtons.add(StartButtons.formixStartButton);
                        }
                    }
    ...
    // Resource counter (food/wood/ore), shown top-left ONLY over the inventory screen (not
    // everywhere), styled as a vertical stack of framed rows similar to the reference screenshot
    // the user provided (icon + number per row, stacked vertically).
    @SubscribeEvent
    public static void onDrawScreenResourcesOverlay(ScreenEvent.Render.Post evt) {
        if (!(evt.getScreen() instanceof InventoryScreen) && !(evt.getScreen() instanceof CreativeModeInventoryScreen))
            return;
        ...
        Resources resources = ResourcesClientEvents.getResources(playerName);
        if (resources == null)
            return;
        ...
        String[] resourceOrder = new String[] { "food", "wood", "ore" };
        int blitY = startY;
        for (String resourceName : resourceOrder) {
            ...
            String resValueStr = resValue + " / " + Resources.MAX_RESOURCE_AMOUNT;

            MyRenderer.renderFrameWithBg(evt.getGuiGraphics(), startX, blitY, rowWidth, rowHeight, frameBgColour);
            MyRenderer.renderIcon(evt.getGuiGraphics(), rl, startX + 6, blitY + (rowHeight - iconSize) / 2, iconSize);
            evt.getGuiGraphics().drawString(
                    MC.font,
                    resValueStr,
                    startX + 6 + iconSize + 6,
    ...
    public static void onRenderOverLay(RenderGuiOverlayEvent.Pre evt) {
        // Hide vanilla health/experience overlays always - they're replaced by the custom
        // heart/level/eidos HUD elements (see renderCustomPlayerHud below) to avoid showing
        // two counters for the same thing side by side.
        // NOTE: EXPERIENCE_BAR covers both the bar AND the level number drawn on top of it in
        // vanilla Minecraft - there is no separate VanillaGuiOverlay for just the level text.
        if (evt.getOverlay() == VanillaGuiOverlay.PLAYER_HEALTH.type()
                || evt.getOverlay() == VanillaGuiOverlay.EXPERIENCE_BAR.type()) {
            evt.setCanceled(true);
            return;
        }
    ...
    // TEMPORARY/SCAFFOLDING custom HUD (see FORMIX_FACTION_LOG.md, added 22.07.2026 per user
    // request) drawn in place of the vanilla health/XP bar we hide above: a heart icon (bottom
    // left), a clock icon showing in-game time of day (bottom right), and a row of three Formix
    // stat counters + an eidos counter, all reading from FormixPlayerProgressionClientEvents.
    // None of this grants real gameplay effects yet - values are only ever set via the
    // /reignofnether formix-stat command until the real skill tree (still being designed by the
    // user in draw.io) and real eidos-gain sources are built.
    @SubscribeEvent
    public static void onRenderCustomPlayerHud(RenderGuiOverlayEvent.Post evt) {
        ...
        int screenWidth = MC.getWindow().getGuiScaledWidth();
        int screenHeight = MC.getWindow().getGuiScaledHeight();
        var progression = FormixPlayerProgressionClientEvents.progression;

        // heart (bottom left) - replaces vanilla health bar, shows current/max HP as text over
        // a heart icon since we don't have a segmented-heart texture yet
        int heartSize = 20;
        int heartX = 4;
        int heartY = screenHeight - heartSize - 4;
        MyRenderer.renderIcon(evt.getGuiGraphics(),
                ResourceLocation.fromNamespaceAndPath(ReignOfNether.MOD_ID, "textures/hud/formix_heart.png"),
                heartX, heartY, heartSize);
        int hp = (int) Math.ceil(MC.player.getHealth());
        int maxHp = (int) Math.ceil(MC.player.getMaxHealth());
        evt.getGuiGraphics().drawString(MC.font, hp + "/" + maxHp, heartX, heartY - 10, 0xFFFFFF);

        // clock (bottom right) - replaces nothing vanilla, just a custom time-of-day readout
        int clockSize = 20;
        int clockX = screenWidth - clockSize - 4;
        int clockY = screenHeight - clockSize - 4;
        MyRenderer.renderIcon(evt.getGuiGraphics(),
                ResourceLocation.fromNamespaceAndPath(ReignOfNether.MOD_ID, "textures/hud/formix_clock.png"),
                clockX, clockY, clockSize);
        long dayTime = MC.level.getDayTime() % 24000;
        // vanilla daytime 0 = 6:00 ingame, ticks 20/sec, 24000 ticks/day
        int inGameHour = (int) (((dayTime + 6000) / 1000) % 24);
        int inGameMinute = (int) (((dayTime + 6000) % 1000) * 60 / 1000);
        ...
        int statSpacing = 60;
        int statsTotalWidth = statSpacing * 3;
        int statsStartX = (screenWidth - statsTotalWidth) / 2;
        int statsY = screenHeight - 65;

        String[] statLabels = new String[] { "Тело: " + progression.bodyLevel, "Разум: " + progression.mindLevel, "Кулак: " + progression.fistLevel };
        for (int i = 0; i < statLabels.length; i++) {
            int statX = statsStartX + i * statSpacing;
            evt.getGuiGraphics().drawString(MC.font, statLabels[i], statX, statsY, 0xAADDFF);
        }
    }
}
